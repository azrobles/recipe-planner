...
@WebMvcTest(TimeUnitController.class)
class TimeUnitControllerTest {

	private static final String BASE_URL = "/api/timeunits";
	...
	void indexControllerTest() throws Exception {
		when(service.index()).thenReturn(List.of(new TimeUnit()));
	...
	void showControllerTest() throws Exception {
		Long id = 1L;
		when(service.show(id)).thenReturn(new TimeUnit(id, "name"));
  ...
  @Test
	void createControllerMethodArgumentNotValidExceptionTest() throws Exception {
		TimeUnit entity = new TimeUnit(null, null);
  ...
  @Test
    ...
    Long id = 1L;
		TimeUnit entity = new TimeUnit(id, null);
